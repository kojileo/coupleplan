...
describe('プラン詳細ページ統合テスト', () => {
  ...
  it('場所URLが複数ある場合、すべて表示される', async () => {
    // 複数の場所を持つプランをモック
    const planWithMultipleLocations = {
      ...mockPlan,
      locations: [
        {
          id: 'loc-1',
    ...
    (api.plans.get as jest.Mock).mockResolvedValue({ data: planWithMultipleLocations });

    render(<PlanDetailPage {...mockParams} />);

    // すべての場所が表示されることを確認
    await waitFor(() => {
      expect(screen.getByText('場所1')).toBeInTheDocument();
      expect(screen.getByText('場所2')).toBeInTheDocument();
    });
  });

  it('場所の名前がない場合、URLのホスト名が表示される', async () => {
    // 名前のない場所を持つプランをモック
    const planWithUnnamedLocation = {
      ...mockPlan,
      locations: [
        {
          id: 'loc-1',
    ...
    (api.plans.get as jest.Mock).mockResolvedValue({ data: planWithUnnamedLocation });
